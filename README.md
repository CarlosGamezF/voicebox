...
When `language` is omitted, `/speak` and the MCP `voicebox.speak` tool use the profile's language (then `en`), and both apply the persisted generation settings (chunk size, crossfade, normalisation) that the desktop form sends. `POST /transcribe` and MCP `voicebox.transcribe` default `model` to the configured STT model when it is already downloaded, otherwise to the currently loaded size. `POST /profiles/{id}/samples` and `POST /transcribe` accept optional `start_s`/`end_s` form fields (seconds) to keep only a window of the clip, and the sample response carries quality `warnings`. For Spanish (`language: "es"`) the text is verbalized before synthesis: numbers, decimals, dates, times, money, percentages, units, ordinals and common abbreviations are spelled out the way they are said ("1.234,56 €" is spoken as "mil doscientos treinta y cuatro euros con cincuenta y seis céntimos"); the stored text is unchanged, and `"verbalize": false` on `/generate` or `/speak` turns it off (retry and regenerate replay the take's setting). `GET /profiles/samples/{id}/analysis` returns the measurements behind those warnings; add `?verify_transcript=true` to transcribe the sample and flag a stored transcript that continues past the audio (every take would start by speaking those words).
