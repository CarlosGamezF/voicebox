...
class _FakeGenerationSettings:
    """Stands in for the persisted GenerationSettings row."""
...
_FAKE_SETTINGS = _FakeGenerationSettings()
...
def captured_request(monkeypatch):
    ...
    # Both surfaces now read the persisted generation settings; the fake DB
    # has no settings row, so serve a fixed stub on both modules.
    monkeypatch.setattr(speak_route, "get_generation_settings", lambda db: _FAKE_SETTINGS)
    monkeypatch.setattr(tools, "get_generation_settings", lambda db: _FAKE_SETTINGS)
    return captured
