...
1. **Idioma.** La app de escritorio ya envía el idioma del perfil (`FloatingGenerateBox.tsx:155-158`), así que "idioma sin fijar" no explica la ruta de escritorio. Sí aplicaba a `POST /speak`, al MCP `voicebox.speak` y a la reproducción de capturas, que forzaban `en`; corregido en esta rama (issue upstream #1049, PR #1054).
